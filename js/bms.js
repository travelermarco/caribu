'use strict';
// ── XiaoXiang / JBD BMS BLE ──────────────────────────────────────────────────
// Service:  0000ff00-0000-1000-8000-00805f9b34fb
// Write:    0000ff02 (Write Without Response)
// Notify:   0000ff01
...
export class BMABLE {
  constructor(onUpdate) {
    ...
    this.data = {
      ...
      temps: [], cells: [], protect: 0,
      fetCharge: true, fetDischarge: true, // assume ON until data arrives
      balance: 0,     // 16-bit bitmask: bit i = cella i+1 in bilanciamento
      cellCount: 0,
      error: null,
    };
  }
  ...
  async reconnect(device) {
    ...
    try {
      ...
      console.warn('BMS auto-reconnect failed:', e);
      return false;
    }
  }
  ...
  async _send(cmd) {
    ...
    try {
      // FF02 is "Write Without Response" per XiaoXiang/JBD BMS protocol.
      // writeValue() is deprecated in Web Bluetooth — use writeValueWithoutResponse().
      await this.wChar.writeValueWithoutResponse(cmd);
    } catch (e) {
      console.warn('BMS write error:', e);
    }
  }
  ...
  _parseFrame(b) {
    ...
    if (cmd === 0x03) { // basic info
      ...
      // Balance bitmask: d[12]=high byte cells 9-16, d[13]=low byte cells 1-8
      // Combined 16-bit: bit 0 = cell 1, bit 15 = cell 16
      this.data.balance = (d[12] << 8) | d[13];

      this.data.protect  = u16(16);
      this.data.cellCount = d[18];
      this.data.soc      = d[19];

      // FET status: bit 0 = CHG MOSFET on, bit 1 = DSG MOSFET on
      if (d.length > 20) {
        this.data.fetCharge    = !!(d[20] & 0x01);
        this.data.fetDischarge = !!(d[20] & 0x02);
      }
